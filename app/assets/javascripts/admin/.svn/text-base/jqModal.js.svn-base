...
(function ($) {
	$.fn.jqm = function (o) {
		var p = {
			...
			trigger : '.jqModal',
			ajax : F,
			ajaxText : '<div class="jqModal-loading"><img src="/assets/loader.gif" alt="loading" /></div>',
			target : F,
			modal : F,
			toTop : F,
			onShow : F,
    ...
    $.fn.jqmIframe = function(o){
            ...
            function openInFrame(hash){
                ...
                var pload = $('<div class="jqModal-loading"><img src="/assets/loader.gif" alt="loading" /></div>');
                ...
                //加载 iframe
                var iframe = $('<iframe src="'+$(hash.c.trigger).data('iframe-src')+'" frameborder="0" width="'+width+'" height="'+height+'"></iframe>').get(0);
    ...
    $.jqmIframe = function(o){
        ...
        $(o.trigger).each(function(index,ele){
            var o = jQuery.extend(tempo, {trigger: ele});
    ...
    $.jqmFullWindow = function(){
        if(window.global_jqModal){
            ...
            hash.w.css('width',$(window).width()-20).css('left',left).css('top',top).css('margin',0);
            wrapper.css('height',$(window).height()-90);
        }
    }
