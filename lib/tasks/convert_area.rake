  ...
  task :convert_area => :environment do
    begin
      ...
      doc.get_elements("//province").each do |pro|
        province = AreaProvince.create(:name => pro.attribute("name").value)
        puts "--"+pro.attribute("name").value
        pro.get_elements("city").each do |ci|
          city = AreaCity.create(:area_province_id => province.id, :name => ci.attribute("name").value)
          puts "------"+ci.attribute("name").value
          ci.get_elements("country").each do |co|
            country = AreaCountry.create(:area_city_id => city.id, :name => co.attribute("name").value)
